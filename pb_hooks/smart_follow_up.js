  ...
  for (const group of activeGroups) {
    ...
    for (const member of members) {
      ...
      try {
        ...
        member.set("last_message_sent_at", now.toISOString().replace('T', ' ').substring(0, 19));
        member.set("status", (completedSeq + 1 >= sequences.length) ? "completed" : "in_progress");
        $app.save(member);

        memberIndex++;
      } catch (memberErr) {
        console.log(`[Smart Follow Up Error] Failed processing member ${member.id} for customer ${customerId}:`, memberErr.message || memberErr, memberErr.stack || "");
      }
    }

    // Update member/sequence counts
    const totalMembers = $app.findRecordsByFilter("follow_up_members", `group = "${groupId}"`, null, 5000, 0);
